    ...
    def get(self, profile, architecture, branch, hardware):
        ...
        try:
            if hardware.strip() != '' and hardware.strip() != '0' and hardware != 0:
                l = self.db.query(
                    'SELECT * from crosswalk.reportsummary WHERE profile=%s AND darchitecture=%s AND branch=%s AND hardware=%s order by qa_id DESC LIMIT 12', profile, architecture, branch, hardware)
                if not l:
