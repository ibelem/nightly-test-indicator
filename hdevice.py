...
class DeviceManagementHandler(tornado.web.RequestHandler):
    def get(self):
        ...
        results = self.db.query("SELECT * FROM crosswalk.device ORDER BY platform ASC")
        #if not entries: raise tornado.web.HTTPError(404)
        self.render("device.htm", title="Crosswalk Nightly Test Device Management", results=results)
...
class DeviceManagementEditHandler(tornado.web.RequestHandler):
    def get(self, entry_id):
        ...
        #if not entries: raise tornado.web.HTTPError(404)
        self.render("deviceupdate.htm", title="Crosswalk Nightly Test Device Management - Edit", results=results, entry_id=entry_id)
...
class DeviceManagementUpdatePostHandler(tornado.web.RequestHandler):
    def post(self, entry_id):
        ...
        self.db = torndb.Connection(
                host=options.mysql_host, database=options.mysql_database,
                user=options.mysql_user, password=options.mysql_password)
        self.db.execute("UPDATE crosswalk.device SET name=%s, type=%s, asset=%s, priority=%s, platform=%s, architecture=%s, sdk=%s, serial=%s, date=now() WHERE id=%s", 
